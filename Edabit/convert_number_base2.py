# Create a function that returns a base-2 (binary) represenation of a base-10
# (decimal) string number. To convert is simple: ((2) meanse base-2 and (10) means
# base-10) 010101001(2) = 1 + 8 + 32 + 128.
# Going from right to left, the value of the most right bit is 1, now from that
# every bit to the left will be two times the value, value of an 8 bit binary numbers
# are (256, 128, 64, 32, 16, 8, 4, 2, 1).
def binary(decimal):
    return bin(decimal)[2:]


a = binary(1)
print(a)

a = binary(5)
print(a)

a = binary(10)
print(a)
